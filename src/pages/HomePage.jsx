...
import { Calendar, MapPin, Sparkles, ExternalLink, ArrowRight, ShieldCheck, Heart, Radio, Activity, CheckCircle2 } from 'lucide-react';
    ...
    <div className="space-y-24 pb-20">
      ...
      <section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden pt-12">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 text-center space-y-8">
          ...
          {/* Sacred Badge */}
          <div className="inline-flex items-center space-x-2 px-4 py-1.5 rounded-full glass-card border border-gold-metallic/40 text-xs tracking-widest text-gold-300 uppercase font-medium shadow-lg">
            <Sparkles className="w-3.5 h-3.5 text-teal-glow animate-pulse" />
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm sm:text-lg lg:text-xl text-teal-300 font-medium tracking-[0.2em] uppercase">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
            ...
            <button
              ...
              className="w-full sm:w-auto px-8 py-4 rounded-full glass-card border border-teal-400/40 hover:border-teal-glow text-teal-300 hover:text-white font-semibold text-sm tracking-widest uppercase transition duration-300"
            >
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center space-y-3 mb-16">
          <div className="text-xs font-semibold text-teal-400 tracking-widest uppercase">
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center space-y-3 mb-16">
          <div className="text-xs font-semibold text-gold-400 tracking-widest uppercase">
            2026 Continuum
          </div>
          <h2 className="font-serif text-3xl sm:text-4xl font-bold gold-gradient-text uppercase">
            ...
            <div 
              ...
              className={`p-6 sm:p-8 rounded-2xl glass-card border transition duration-300 flex flex-col md:flex-row md:items-center justify-between gap-6 ${
                item.status === 'Register Now' 
                  ? 'border-gold-metallic/50 shadow-2xl shadow-gold-500/10 bg-[#0A131A]' 
                  : 'border-white/10 opacity-80'
              }`}
            >
              <div className="space-y-2">
                <div className="flex items-center space-x-3">
                  <span className="px-3 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase bg-teal-500/20 text-teal-300 border border-teal-500/30">
                    {item.date}
                  </span>
              ...
              <div className="shrink-0">
                {item.status === 'Register Now' ? (
                  ...
                  <span className="text-xs text-slate-500 font-medium px-4 py-2 rounded-full border border-white/5">
                    Completed · Recorded
                  </span>
                )}
              </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="glass-card p-8 sm:p-12 rounded-3xl border border-gold-metallic/30 space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            {/* Ticket Split */}
            <div className="space-y-4 p-6 rounded-2xl bg-[#060B0E] border border-gold-metallic/20">
              <div className="font-serif text-base font-bold text-gold-300 flex items-center justify-between">
                <span>Ticket Revenue Distribution</span>
                <span className="text-xs font-sans text-teal-400">Practitioners First</span>
              </div>
              <div className="space-y-3">
                {SENSORIUM_DATA.revenueSplit.tickets.map((t, idx) => (
                  <div key={idx} className="flex items-center justify-between text-xs pb-2 border-b border-white/5">
                    <div>
                      <div className="text-slate-200 font-medium">{t.label}</div>
                      <div className="text-[11px] text-slate-500">{t.desc}</div>
                    </div>
                    <div className="font-bold text-gold-400 text-sm">{t.pct}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Vendor Split */}
            <div className="space-y-4 p-6 rounded-2xl bg-[#060B0E] border border-teal-500/20">
              <div className="font-serif text-base font-bold text-teal-300 flex items-center justify-between">
                <span>Vendor & Sponsorship Split</span>
                <span className="text-xs font-sans text-gold-400">Sustainable Ecosystem</span>
              </div>
              <div className="space-y-3">
                {SENSORIUM_DATA.revenueSplit.vendors.map((v, idx) => (
                  <div key={idx} className="flex items-center justify-between text-xs pb-2 border-b border-white/5">
                    <div>
                      <div className="text-slate-200 font-medium">{v.label}</div>
                      <div className="text-[11px] text-slate-500">{v.desc}</div>
                    </div>
                    <div className="font-bold text-teal-400 text-sm">{v.pct}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="max-w-3xl mx-auto px-4 text-center space-y-6">
        ...
        <div className="space-y-2">
          <h2 className="font-serif text-2xl sm:text-3xl font-bold text-slate-100 uppercase">
        ...
        {leadSubmitted ? (
          <div className="p-4 rounded-xl bg-teal-500/20 border border-teal-400 text-teal-300 text-xs font-medium flex items-center justify-center space-x-2">
          ...
          <form onSubmit={handleLeadSubmit} className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            <input
              ...
              className="flex-1 px-4 py-3 rounded-full bg-[#0E161C] border border-gold-metallic/20 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-teal-400"
            />
            <input
              type="email"
              required
